...
class WrappedAnimatedInteractableCardView extends Component {
    ...
    scrollHostedViewToTop(params) {
        if (Platform.OS === 'ios') {
            NativeViewManager.scrollHostedViewToTop(ReactNative.findNodeHandle(this), params);
        }
    }

    attachScrollViewIfNeeded(params) {
        if (Platform.OS === 'ios') {
            NativeViewManager.attachScrollViewIfNeeded(ReactNative.findNodeHandle(this), params);
        }
    }
}
